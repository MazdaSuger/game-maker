...
| 🧑 キャラ・表情差分 | キャラごとに複数の表情（立ち絵）を登録・切り替え。1画面に最大3人（左/中央/右）配置 |
| 🎒 アイテム管理 | 入手 / 使用 / 破棄。使用で消費するかをアイテムごとに切替。絵文字＋PNG画像アイコン、入手演出（アイコン＋説明） |
...
| 🔊 | 効果音(SE) | 効果音を一度だけ再生 |
| 🎞 | エンドロール | セリフ枠が消え、縦スクロールでスタッフロール（スキップ可/不可を選択） |
| 💬 | セリフ | 立ち絵の位置（左/中央/右）を指定。1画面に最大3人 |
| 🔤 | 名前入力 | 入力値を文字列変数へ格納 |
| 🔢 | 変数操作 | 代入 / 加算 / 減算 / 乗算 / 反転 |
...
- **コンポーネントの非表示**：セリフ枠 / 立ち絵 / ゲージ / アイテム / メニューを
  チェックひとつで個別に隠せます（セリフ枠を隠した場合は画面クリックで進行）
